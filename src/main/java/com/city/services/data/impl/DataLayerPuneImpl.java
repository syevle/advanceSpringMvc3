...
import com.city.factories.pune.HibernatePuneDaoFactory;
import com.city.model.obj.pune.Site;
import com.city.persistence.GenericDAO;
import com.city.services.data.DataLayerPune;
/** 
 * Data layer.
 * @author santosh yevle
 */
@Component
public class DataLayerPuneImpl implements DataLayerPune {
	/** Singleton reference to this class. */
	private static DataLayerPune instance;
	/** map lock. */
 ...
 /** Sessionfactory in use. Filled in by Spring. */ 
	...
	public void setApplicationContext(ApplicationContext ctxt) {
		DataLayerPuneImpl.context = ctxt;
	}
	...
	* @return GenericDAO<T, ?> object
		...
		synchronized (daoMapLock) {
    		if (daoMap == null) {
    			daoMap = new ConcurrentHashMap<Class<?>, GenericDAO<?, ?>>(); 
	 	   		daoMap.put(Site.class, HibernatePuneDaoFactory.getSiteDao());
    		}
		 }
    ...
    public void deleteSite(final Long id)  {
        HibernatePuneDaoFactory.getSiteDao().delete(loadSite(id));
    }
    ...
    public Site loadSite(final Long id) {
        return HibernatePuneDaoFactory.getSiteDao().load(id);
    }
    /**
     * Loads the given Object.
     * @param id Id to load
     ...
     */
     public Site getSite(final Long id) {
        return HibernatePuneDaoFactory.getSiteDao().get(id);
    }  
    /** Returns a singleton instance of this class.
     * @return an singleton instance
     */
    public static synchronized DataLayerPune getInstance() {
        
        if (instance == null) {
        	if (context == null) {
        		throw new IllegalStateException("Context is null. Make sure Spring is initialized.");
        	}
     		instance = (DataLayerPune) context.getBean("dataLayerPuneImpl");
        }
